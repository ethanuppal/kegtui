...
while true; do
    read -p "=> Proceed with installation? " yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
printf "\e[0m"
tar xf "$WRAPPER_DOWNLOAD_PATH" --directory "$ARTIFACTS_FOLDER" || fail "Failed to extract wrapper from download"
mv "$ARTIFACTS_FOLDER/${WRAPPER_DOWNLOAD_NAME%.tar.xz}.app" "$WRAPPER_FOLDER" || fail "Failed to install wrapper"
printf "\e[1m"
